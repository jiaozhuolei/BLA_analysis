...
def generate_plotly_color(value,color1=[0,255,0],color2=[255,0,255],cmin=0,cmax=99):
    if value > cmax:
        value = cmax
...
def generate_go_ba(area = "root",o=0.05,c="grey"):
    if area == "root":
        obj_file = r"D:\projectome\allobj\997.obj"
    else:
        with open(r"D:\python\core\obj_dic.pkl","rb") as f:
            obj_dic = pickle.load(f)
        
        for item_area in obj_dic:
            if item_area == area:
                obj_file = obj_dic[item_area]
...
def generate_go_layout(title="Parametric Plot",eye_from="front",**kwargs):
    front_view = dict(x=-1.5,y=0,z=0)
    top_view = dict(x=0,y=-1.5,z=0)
    left_view = dict(x=0,y=0,z=-1.5) 
    # se_view = dict(x=-1.25, y=-2, z=-1.25)
    se_view = dict(x=-1.25, y=-1, z=-1.25)
    if eye_from == "front":
        eye = front_view
    if eye_from == "left":
        eye = left_view
