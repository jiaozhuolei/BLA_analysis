...
#for fig1 f
dfdata = read.table("BLA_neurons_0819_top602.csv",row.names = "X",header = TRUE,sep = ",")

#for figs 5
# dfdata = read.table("all_neurons_terminal_terminal_number_Supplementary_fig3_sort.csv",row.names = "X",header = TRUE,sep = ",")

dfcol = read.table("BLA_fourtype_plot_60_4.csv",row.names = "X",header = TRUE,sep = ",")
dflabel = read.table("BLA_geno_location2.csv",row.names = "X",header = TRUE,sep = ",")

dfdist = read.table("morphology_matrix.csv",row.names = "X",header = TRUE,sep = ",")
d = as.dist(dfdist)
...
geno_colors <- c('Pkc'='red','Crh'='#00ffff','Vglut1'='#107010','Sst'='#0000ff')
...
pdf("Fig1f3.pdf", width = 9, height = 13)
